...
class RGBImage:
    ...
    def __init__(self, pixels):
        ...
        if not isinstance(pixels, list) or len(pixels) == 0:
            raise TypeError()
        if any([not isinstance(row, list) or len(row) == 0 for row in pixels]):
            raise TypeError()
        num_cols = len(pixels[0])
        if any([len(row) != num_cols for row in pixels]):
            raise TypeError()
        if any([not isinstance(pixel, list) or len(pixel) != 3 for row in pixels for pixel in row]):
            raise TypeError()
        if any([not all([isinstance(value, int) and 0 <= value <= 255 for value in pixel]) for row in pixels for pixel in row]):
            raise ValueError()
        self.pixels = pixels
    ...
    def get_pixels(self):
        ...
        True
        """
        return [[[rgb for rgb in cols] for cols in rows] for rows in self.pixels]
    ...
    def set_pixel(self, row, col, new_color):
        ...
        if row >= self.num_rows or col >= self.num_cols:
            raise ValueError()
        if not isinstance(new_color, tuple) or len(new_color) != 3 or any([not isinstance(num, int) for num in new_color]):
            raise TypeError()
...
class ImageProcessingTemplate:
    ...
    def negate(self, image):
        ...
        """
        return RGBImage([[[255 - pixel for pixel in cols] for cols in row] for row in image.copy().get_pixels()])
    ...
    def grayscale(self, image):
        ...
        """
        return RGBImage([[[sum(cols) // 3 for pixel in cols] for cols in row] for row in image.copy().get_pixels()])
    ...
    def rotate_180(self, image):
        ...
        """
        return RGBImage([[pixel for pixel in row[::-1]] for row in image.copy().get_pixels()[::-1]])
    ...
    def get_average_brightness(self, image):
        ...
        86
        """
        # return sum([sum(image.get_pixel(row, col)) // 3 for row in image.get_pixels() for col in row]) // (image.size()[0] * image.size()[1])
        width, height = image.size()
        return sum([sum(image.get_pixel(row, col)) // 3 for row in range(height) for col in range(width)]) // (width * height)
    ...
    def adjust_brightness(self, image, intensity):
        ...
        if not isinstance(intensity, float):
            raise TypeError()
        return RGBImage([[[max(min(int(pixel * intensity), 255), 0) for pixel in cols] for cols in row] for row in image.copy().get_pixels()])
...
class PremiumImageProcessing(ImageProcessingTemplate):
    ...
    def pixelate(self, image, block_dim):
        ...
        """
        width, height = len(image.get_pixels()[0]), len(image.get_pixels())
        pixelated_image = [row[:] for row in image.get_pixels()] 
        for row in range(0, height, block_dim):
            for col in range(0, width, block_dim):
                ...
                blue_total = 0
                count = 0
                for i in range(row, min(row + block_dim, height)):
                    for j in range(col, min(col + block_dim, width)):
                        ...
                        blue_total += b
                        count += 1
                r_avg = red_total // count
                g_avg = green_total // count
                b_avg = blue_total // count
                for i in range(row, min(row + block_dim, height)):
                    for j in range(col, min(col + block_dim, width)):
                        pixelated_image[i][j] = [r_avg, g_avg, b_avg]
    ...
    def edge_highlight(self, image):
        ...
        """
        grayscale = [[sum(pixel) // 3 for pixel in row] for row in image.get_pixels()]
        kernel = [ \
            [-1, -1, -1], \
            [-1,  8, -1], \
            [-1, -1, -1] \
        ...
        def apply_kernel(x, y):
            return sum([kernel[i][j] * grayscale[x+i-1][y+j-1] \
                for i in range(3) for j in range(3) \
                if 0 <= x+i-1 < len(grayscale) and 0 <= y+j-1 < len(grayscale[0])])
        highlighted = [[0]*len(grayscale[0]) for _ in range(len(grayscale))]
        for i in range(len(grayscale)):
            for j in range(len(grayscale[0])):
                value = apply_kernel(i, j)
                highlighted[i][j] = [max(0, min(255, value))]*3
...
class PremiumPlusMultimediaProcessing(PremiumImageProcessing):
    ...
    def reverse_song(self, audio):
        ...
        if not isinstance(audio, AudioWave):
            raise TypeError()
        return AudioWave([sample for sample in audio.wave[::-1]])
    ...
    def reverb(self, audio):
        ...
        if not isinstance(audio, AudioWave):
            raise TypeError()
        echo_filter = [1.0, 0.8, 0.6, 0.4, 0.2]
        return AudioWave([round(max(min(sum(audio.wave[i - j] * echo_filter[j] \
                        for j in range(min(i + 1, len(echo_filter)))), 32767), -32768)) if i >= 4 \
                        else audio.wave[i] for i in range(len(audio.wave))])
    ...
    def clip_song(self, audio, start, end):
        ...
        if end <= start:
            return AudioWave([])
        beginning_clip = int(len(audio.wave) * (start / 100))
        end_clip = int(len(audio.wave) * (end / 100))
        return AudioWave(audio.wave[beginning_clip:end_clip+1])
...
class ImageKNNClassifier:
    ...
    def distance(self, image1, image2):
        ...
        if image1.size() != image2.size():
            raise ValueError()
        distance_num = (sum(list(map(lambda x, y: (x - y)**2, \
                    [pixel for cols in image1.pixels for row in cols for pixel in row], \
                    [pixel for cols in image2.pixels for row in cols for pixel in row]))))**0.5
        return distance_num
    ...
    def predict(self, image):
        ...
        if not self.data:
            raise ValueError()
        # sorted_dists = [(self.distance(tup[0], image), tup[1]) for tup in self.data].sort()
        # k_nearest_labels = [label for _, label in sorted_dists[:self.k_neighbors]]
        # return self.vote(k_nearest_labels)
        distances = list(map(lambda tup: (self.distance(tup[0], image), tup[1]), self.data))
        sorted_distances = sorted(distances)
        k_nearest_labels = list(map(lambda pair: pair[1], sorted_distances[:self.k_neighbors]))
        return self.vote(k_nearest_labels)
